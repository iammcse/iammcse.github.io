...
console.log("offsetMilliSecondsValue: " + offsetMilliSecondsValue ); 
  ...
  if (now.getTime() > morningSession.getTime() && now.getTime() <= afternoonSession.getTime() )    {
    myInputTime = afternoonSession - afternoon_offsetMilliSecondsValue;  // exact time to lauch 
...
var displayTarget = new Date(myInputTime); // Creat Object using exact time launch 
console.log("displayTarget is : " + myInputTime);
...
//     }
  ...
  document.title = hours + "h" + minutes + "m" + seconds + "s " + titleDate; // countdown + exact time
  //+ milliseconds +"ms"
